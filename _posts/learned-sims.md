...
We know about engineered systems and how to build them: we understand the problem,
break it down into it's core components, and solve each of the invidual pieces,
and combine the pieces together. This works pretty well and it let us put a man
on the Moon for example.
We know about embodied intelligences and what it's like to be one:
we can handle a wide variety of settings that the real world throws at us.
But we are just starting to figure out how to develop intelligences in silico.
This combination of requirements for a system that must be highly flexible,
but highly reliable---that represents an extremely challenging exciting *frontier* in technology today.

We have fairly well-established practices for 
...
Simulation is actually a well-established tool in both domains of engineering, and skill acquisition---it's been used both to [design complex][defenseone] [fighter jets][popmech] and to [teach the human pilots][flightsim] that fly them.
...
It's going to offer an indespensiable proxy for reality that we use for
development, training, and evaluation to make sure the systems we are deploying
are actually safe. The best robotics companies will have the best simulators,
because that will be the cheapest, quickest, and safest way to iterate.
