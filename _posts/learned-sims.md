...
> This post represents some of my thoughts on **learned simulators** specifically. It is a spin-off from my other post on [The Future of Robot Learning](/robot-future), which provides some more contextualization.
...
Some examples of datasets which would raise the waterline are Tesla's fleet data, Google's JFT, OpenAI's 400M text image pairs, which are all proprietary.
There are good legal and financial incentive reasons these organizations don't release their data.
But it would be interesting to try to figure out how you could create powerful open datasets. For some
pointers, Andrej has some recent [tweets](https://twitter.com/karpathy/status/1363973271717171200) [about](https://twitter.com/karpathy/status/1365511769255342084) this. [The Pile](https://pile.eleuther.ai/) looks like an interesting work in this space. Maybe something like [Numerai Signals](https://signals.numer.ai/)
would work, where we could compensate people for providing their useful data to a common pool.
...
people can solve their own tasks by reusing the optimization, but within a limited scope.
With more powerful and general unsupervised models like GPT-3, it's likely that reusable learned models will
start raising the waterline for many more domains we care about.

## Learned simulators

Learned simulators seem to provide a good framing on how we might develop large predictive models in robotics, and in how we might raise the waterline.
...
you are failing and then embedding similar scenarios in the simulator to create behavioral unit tests. (Part of this applies to traditional simulators as well, but learned simulators greatly reduce the friction here.)
